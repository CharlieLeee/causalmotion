...
do
    ...
    do
        # pretrained model
        exp="contrast_dual_style_multiseed_${seed}_OOD7"
        model_dir="./models/$dataset/$exp/$step/$irm/$finetune/$seed"
        DIR="--tfdir ft_runs/$dataset/$exp/$step/$irm/$finetune/$seed"

        TRAINING="--num_epochs $epoch-0-0-0-0-$p6 --batch_size $bs --finetune $finetune --lrinteg $lrinteg --contrastive $contrastive --irm $irm --decoder_bottle $dbottle"
